...
time2rescue     ;; time taken to rescue all random tagerts
plotlist        ;; For plotting
pen_list        ;; For plotting
current_pen     ;; For plotting
send_to_center? ;; For plotting
done            ;; Have we found them all?
g_action        ;; Global UAV Action command
;steerrange     ;; Future
]
...
to setup
  ...
  set greedy_team 1
  set  num-found 0
  set time2rescue lost-target

  auto-plot-on
  
  set plotlist ( list  "run1" "run2" "run3" "run4" "run5" "run6" "run7" "run8" "run9" "run10" "run11" "run12" "run13" "run14" "run15" "run16" "run17" "run18" "run19" "run20" "run21" "run22" "run23" "run24" "run25" "run26" "run27" "run28" "run29" "run30" "run31" "run32" "run33" "run34" "run35" "run36" "run37" "run38" "run39" "run40" "run41" "run42" "run43" "run44" "run45" "run46" "run47" "run48" "run49" "run50" "run51" "run52" "run53" "run54" "run55" "run56" "run57" "run58" "run59" "run60" "run61" "run62" "run63" "run64" "run65" "run66" "run67" "run68" "run69" "run70" "run71" "run72" "run73" "run74" "run75" "run76" "run77" "run78" "run79" "run80" "run81" "run82" "run83" "run84" "run85" "run86" "run87" "run88" "run89" "run90" "run91" "run92" "run93" "run94" "run95" "run96" "run97" "run98" "run99" "run100" )

  foreach ( list  "run1" "run2" "run3" "run4" "run5" "run6" "run7" "run8" "run9" "run10" "run11" "run12" "run13" "run14" "run15" "run16" "run17" "run18" "run19" "run20" "run21" "run22" "run23" "run24" "run25" "run26" "run27" "run28" "run29" "run30" "run31" "run32" "run33" "run34" "run35" "run36" "run37" "run38" "run39" "run40" "run41" "run42" "run43" "run44" "run45" "run46" "run47" "run48" "run49" "run50" "run51" "run52" "run53" "run54" "run55" "run56" "run57" "run58" "run59" "run60" "run61" "run62" "run63" "run64" "run65" "run66" "run67" "run68" "run69" "run70" "run71" "run72" "run73" "run74" "run75" "run76" "run77" "run78" "run79" "run80" "run81" "run82" "run83" "run84" "run85" "run86" "run87" "run88" "run89" "run90" "run91" "run92" "run93" "run94" "run95" "run96" "run97" "run98" "run99" "run100" ) [ create-temporary-plot-pen ? ]
  set-current-plot-pen ( item current_pen plotlist )
  ...
  set-default-shape hangers "house"
  ...
  launch-lost-targets lost-target
  launch-random-ants random-team Goto_Center?
  launch-square-search square-team
  set agentcounter greedy_team
  ...
  create-hangers 1 [  ;; setup house icon
                set size 1
                set color blue
...
to reset-all
   ...
   set  num-found 0
   set time2rescue lost-target
   set current_pen current_pen + 1
   show item current_pen plotlist
   set-current-plot-pen ( item current_pen plotlist )
...
to setup-sensor
  ...
  ask patches in-radius ( 4 * ( sensor-range / sensorscale ) ) 
    [
      ...
      set random-probability random-probability * ( 1 - Z )
    ]
end
...
to recolor-patch  ;; patch procedure
  ...
  if show-probability = 1  [ if pcolor != white [ set pcolor scale-color lime random-probability   0 0.001]  ]
  if show-probability = 2  [ if pcolor != white [ set pcolor scale-color red (target-probability * 100000)  p-min p-max ] ]
  if show-probability = 3  [ if pcolor != white [ set pcolor scale-color green square-probability  0 .001]  ]
  if show-probability = 4  [ if pcolor != white [ set pcolor scale-color red parallel-probability  0 .001]  ]
  if show-probability = 5  [ if pcolor != white [ set pcolor scale-color yellow  home-prob  0 1000] ] 
  
end


;; Advanced ants config
to launch-random-ants [number sendtocenter?]
  create-random-ants number [  ;;setup random ant-like agents
    set size 2
    set color 5 ;cyan
    setxy (max-pxcor * 0.9 - random 5) (max-pycor * 0.1 - random 5)
...
to search-like-ant 
  ...
  if true[
    ifelse patch-ahead (( sensor-range / sensorscale ) * steerrange ) != nobody [
      ask patch-ahead (( sensor-range / sensorscale ) * steerrange ) [ set prob-ahead random-probability ]]
    [ set prob-ahead -1 ]
    ;set turn "no"
    foreach [ 5 15 20 25 30 45 60 90 ][
      ...
      ifelse patch-right-and-ahead ? (( sensor-range / sensorscale ) * steerrange ) != nobody [
        ...
        [
          ...
          set prob-right random-probability 
          ]
        ]
      [ set prob-right -1.0 ]
    ...
    foreach [ 5 15 20 25 30 45 60 90 ][
      ...
      ifelse patch-left-and-ahead ? (( sensor-range / sensorscale ) * steerrange ) != nobody [
        ...
        [ 
          ...
          set prob-left random-probability 
        ]
      ]
      [ set prob-left 0 ]
  ...
  set turn "no"
  if false[
    show max-one-of patches [random-probability]
    show max [random-probability] of patches
  ]
end


;; Poor mans expanding square search
to square-search
  ;set ticker ticks
  ifelse heading = desired-heading [
    if ticks > ticker [
      ifelse (turncount mod 2) = 0 [set turncount turncount + 2 set ticker ticker + ( turncount + 3 )]
        [set turncount turncount + 2 set ticker ticker + ( turncount + 3 )  
        ]
      set desired-heading desired-heading - 90
      if desired-heading < 0 [ set desired-heading desired-heading + 360 ]
    ]
  ]
  [
    let temp-angle subtract-headings heading desired-heading
    ifelse temp-angle = 0 [] [  ifelse temp-angle >= 0 
      [ ifelse ( temp-angle < turn-rate) [ lt temp-angle] [ lt turn-rate ] ] 
      [ ifelse ( temp-angle < turn-rate) [ rt temp-angle] [ rt turn-rate ] ] 
    ]
  ]
end
...
;; Not complete parallel search 
to parallel-search
  ifelse action = "search"
  [ ifelse heading = desired-heading[
    if patch-ahead (( sensor-range / sensorscale ) * steerrange ) = nobody [
      ifelse desired-heading = 0
        [ set desired-heading 180 ]
        [ set desired-heading 0]
      ifelse last-turn-L?
        [ set last-turn-L? false]
        [ set last-turn-L? true ]
    ]
  ]
  [
  ...
  if xcor > max-pxcor - 1 [set action "go-home"]]
  [ send-me-home ]
end
...
to normalise
  if show-probability = 1   [   ;;  Normalise target likelyhood array
    let total sum [random-probability] of patches
    ask patches [ set random-probability random-probability * 1 / total ]
  ]
  if show-probability = 2   [   ;;  Normalise target likelyhood array
    let total sum [target-probability] of patches
    ask patches [ set target-probability target-probability * 1 / total ]
  ]
  if show-probability = 3   [   ;;  Normalise target likelyhood array
    let total sum [square-probability] of patches
    ask patches [ set square-probability square-probability * 1 / total ]
  ]
  if show-probability = 4   [   ;;  Normalise target likelyhood array
    let total sum [parallel-probability] of patches
    ask patches [ set parallel-probability parallel-probability * 1 / total ]
  ]
...
;; Launch UAVs
  if agentcounter != 0 [ 
    every 0.05      
      [
        launch-greedy-uavs 1 Goto_Center? agentcounter  ;; Spawn UAV
        set agentcounter agentcounter - 1
        ask greedy-uavs [ ifelse trails? [pd][pu] ]  ;; Set agent trails
      ]
  ]
  ...
  every 1 [
    ifelse g_action = "go-home"
      [ 
        let num-home count greedy-uavs with [action = "home"] 
        
        ;[ distance (max-one-of greedy-uavs [distance myself] ) < 2 of hanger 0 
        if num-home = count greedy-uavs
          [
            export-plot "Time to rescue" user-new-file
            user-message ( word " Testing Complete!" )
          ]
        
      ]
      [
        ifelse num-found >= lost-target 
          [ 
            ifelse current_pen = max_pen - 1
              [ 
                ask greedy-uavs [ set action "go-home" ]
                ask square-uavs [ set action "go-home" ]
                ask random-ants [ set action "go-home" ]
                setup-home 
              ]
              [ 
                set greedy_team greedy_team + 1 
                if greedy_team > max_agents [ set greedy_team 1 ]
                reset-all 
              ]  
          ] 
          [  ]   
     ...
     ask random-ants 
     [ 
       if searchable? [ look-for-target ] ;; Look for targets in range
     ...
     [ 
       ...
       [   
         ...
         ifelse action = "search"
         [ 
           search-greedy       ;; Search for targets
           fd speed            ;; Move Forward t
           do-update           ;; update probability map with sensor data
           
           ask patches in-radius ( 4 * sensor-range ) [ if ( PDF? and not background? ) [ recolor-patch ] ]  ;; show updated data
     ...
     [ 
       ...
       ifelse g_action = "centre" [ goto-point ( target_x ) + 5 ( target_y ) - 5 ] ;; Send the agents to last known position
       [
         if g_action = "search"
         [ square-search      ;; Do deterministic expanding square
           fd speed           ;; Move Forward t
           do-update          ;; Keep track of PDF for use calculating the information gain
         ] 
       ]
     ]
     
     every 0.5 [ if do_normalise [ normalise ] ]          ;; Normalise the posterior     
     every 0.5 [ diffuse target-probability prediction ]  ;; Predict the target likelyhood
